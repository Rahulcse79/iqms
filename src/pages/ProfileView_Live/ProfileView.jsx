...
export default function ProfileView({ queryValue = '', category = '' }) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      const promises = [
        dispatch(fetchPersonalData(queryValue, category)),
        dispatch(getRankHistory(queryValue, category, 1)),
        dispatch(getTradeHistory(queryValue, category, 1)),
        dispatch(getPostingHistory(queryValue, category, 1)),
      ];
