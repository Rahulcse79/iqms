...
function safeSavePendingToStorageNew(pendingWith, items) {
  try {
    ...
    window.dispatchEvent(new CustomEvent('pendingQueriesUpdated', { 
...
export const generatePenWithCode = (activeRole, level) => {
  ...
  console.log(`📝 Generated PEN_WITH: ${penWith} for module: ${activeRole.MODULE}, level: ${level}`);
  return penWith;
};
 ...
 */
export const fetchPendingQueriesNew = ({ activeRole, level }) => async (dispatch) => {
  const penWith = generatePenWithCode(activeRole, level);
  
  dispatch({
    type: PENDING_QUERY_REQUEST,
    meta: { pendingWith: penWith, apiVersion: "NEW", level, activeRole: activeRole.PORTFOLIO_NAME },
  });
  ...
  try {
    ...
    console.log(`✅ Pending queries (NEW API) response for ${penWith}:`, data);
    ...
    console.log(`✅ Successfully processed ${items.length} pending queries for ${penWith}`);
    return { pendingWith: penWith, items: items, apiVersion: "NEW", level };

  } catch (error) {
    console.error(`❌ Failed to fetch pending queries (NEW API) for ${penWith}:`, error);
 ...
 * @param {Object} activeRole - Active role object
 */
export const fetchAllPendingQueriesForRole = (activeRole) => async (dispatch) => {
  if (!activeRole) {
    console.error("❌ No active role provided for pending queries fetch");
    return { success: false, error: "No active role" };
  }

  console.log(`🚀 Fetching all pending queries for role: ${activeRole.PORTFOLIO_NAME}`);
  ...
  try {
    ...
    responses.forEach((response, index) => {
      ...
      if (response.status === 'fulfilled') {
        console.log(`✅ Level ${level} (${penWith}): ${response.value.items.length} queries`);
        results.push({
          level,
          penWith,
          success: true,
        ...
        console.error(`❌ Level ${level} (${penWith}) failed:`, response.reason);
        results.push({
          level,
          penWith,
          success: false,
    ...
    console.log(`📊 Pending queries fetch completed: ${successful}/${levels.length} successful`);
 ...
 */
export const refreshPendingQueriesNew = ({ activeRole, level }) => async (dispatch) => {
  const penWith = generatePenWithCode(activeRole, level);
...
export const loadPendingQueriesFromStorageNew = (pendingWith) => (dispatch) => {
  try {
    ...
    console.log(`📦 Loading pending queries (NEW API) from storage for ${pendingWith}: ${items.length} items`);
    ...
    console.error("❌ Failed to load pending queries (NEW API) from storage:", error);
...
export const getAllPendingCountsForRole = (activeRole) => {
  ...
  try {
    const creator = getPendingQueriesCountNew(generatePenWithCode(activeRole, "1"));
    const verifier = getPendingQueriesCountNew(generatePenWithCode(activeRole, "2"));  
    const approver = getPendingQueriesCountNew(generatePenWithCode(activeRole, "3"));
