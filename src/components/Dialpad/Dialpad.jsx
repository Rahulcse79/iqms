...
export default function Dialpad() {
  ...
  const init = useMemo(() => {
    ...
    const w = clamp(saved?.w ?? DEFAULT_W, MIN_W, Math.min(MAX_W, window.innerWidth));
    const h = clamp(
      saved?.h ?? DEFAULT_H,
      MIN_H,
      Math.min(MAX_H, Math.max(MIN_H, window.innerHeight - topbarBottom - PADDING))
    );
    const xDefault = Math.max(0, window.innerWidth - w - PADDING);
    const yDefault = Math.max(topbarBottom, window.innerHeight - h - PADDING);
    const x = clamp(saved?.x ?? xDefault, 0, Math.max(0, window.innerWidth - w));
    const y = clamp(saved?.y ?? yDefault, topbarBottom, Math.max(topbarBottom, window.innerHeight - h));
  ...
  const clampAndSetSize = useCallback(
    (nw, nh, x = pos.x, y = pos.y) => {
      ...
      const h = clamp(nh, MIN_H, Math.min(MAX_H, Math.max(MIN_H, window.innerHeight - topbarBottom - PADDING)));
      setSize({ w, h });
  ...
  const pointerMoveDrag = useCallback((ev) => {
  ...
  const pointerUpDrag = useCallback((ev) => {
    draggingRef.current = false;
    setDragging(false);
    document.body.style.userSelect = "";
    try {
      // release pointer capture (safe if headerRef exists)
      if (headerRef.current && ev && ev.pointerId != null && headerRef.current.releasePointerCapture) {
        headerRef.current.releasePointerCapture(ev.pointerId);
      } else if (headerRef.current && dragRef.current.pointerId != null && headerRef.current.releasePointerCapture) {
  ...
  const onHeaderPointerDown = useCallback((e) => {
    ...
    setDragging(true);
    dragRef.current = { sx: e.clientX, sy: e.clientY, ox: pos.x, oy: pos.y, pointerId: e.pointerId };
    document.body.style.userSelect = "none";
    // pointer capture so move events stay with this element
    try { headerRef.current?.setPointerCapture?.(e.pointerId); } catch {}
  ...
  const pointerMoveResize = useCallback((ev) => {
    if (!resizingActiveRef.current) return;
    const dw = ev.clientX - resizingRef.current.sx;
    const dh = ev.clientY - resizingRef.current.sy;
    clampAndSetSize(resizingRef.current.sw + dw, resizingRef.current.sh + dh, pos.x, pos.y);
  }, [clampAndSetSize, pos.x, pos.y]);

  const pointerUpResize = useCallback((ev) => {
    resizingActiveRef.current = false;
    setResizing(false);
    document.body.style.userSelect = "";
    try {
      if (resizeRefElm.current && ev && ev.pointerId != null && resizeRefElm.current.releasePointerCapture) {
        resizeRefElm.current.releasePointerCapture(ev.pointerId);
      } else if (resizeRefElm.current && resizingRef.current.pointerId != null && resizeRefElm.current.releasePointerCapture) {
        resizeRefElm.current.releasePointerCapture(resizingRef.current.pointerId);
      }
    } catch {}
  ...
  const onResizePointerDown = useCallback((e) => {
    ...
    setResizing(true);
    resizingRef.current = { sw: size.w, sh: size.h, sx: e.clientX, sy: e.clientY, pointerId: e.pointerId };
    document.body.style.userSelect = "none";
    try { resizeRefElm.current?.setPointerCapture?.(e.pointerId); } catch {}
  ...
  const resetPosition = () => {
    ...
    const h = clamp(DEFAULT_H, MIN_H, Math.min(MAX_H, window.innerHeight - topbarBottom - PADDING));
    const x = Math.max(0, window.innerWidth - w - PADDING);
    const y = Math.max(topbarBottom, window.innerHeight - h - PADDING);
    setSize({ w, h });
    setPos({ x, y });
    ...
    >
      ...
      >
        ...
        <div className="dialpad__header-actions">
          ...
          <button className="dp-btn dp-btn-ghost" onClick={maximize} aria-label="Reset size & position" title="Reset">
            ⤢
          </button>
        </div>
      </div>
...
function Keypad({ api }) {
  ...
  return (
    <div className="dialpad__keygrid" role="group" aria-label="Dialpad keys">
      {keys.map((k) => (
        <button key={k} className="dp-key" onClick={() => api.appendDigit(k)} aria-label={`Digit ${k}`}>{k}</button>
      ))}
    </div>
  );
}
...
function CallButton({ state }) {
  ...
  return (
    <button className="dp-btn dp-btn-primary" onClick={fakeOutbound} disabled={!canCall} aria-label="Call">Call</button>
  );
}
  ...
  return (
    <div className="dialpad__incall">
      <div className="dialpad__incall-row">
        <button className="dp-btn dp-btn-secondary" onClick={muted ? onUnmute : onMute} disabled={muted ? !can.unmute : !can.mute}>{muted ? "Unmute" : "Mute"}</button>
        <button className="dp-btn dp-btn-secondary" onClick={can.hold ? onHold : onResume} disabled={!(can.hold || can.resume)}>{can.hold ? "Hold" : "Resume"}</button>
        <button className="dp-btn dp-btn-warning" onClick={handleBlind} disabled={!can.blindTransfer}>Blind Transfer</button>
      </div>
      <div className="dialpad__incall-row">
        <button className="dp-btn dp-btn-warning" onClick={handleLine} disabled={!can.lineTransfer}>Line Transfer</button>
        <button className="dp-btn dp-btn-danger" onClick={onHangup} disabled={!can.hangup}>Disconnect</button>
      </div>
    </div>
  );
}
